...
def main():
    ...
    with model_tab2:
        ...
        with col1:
            ...
            st.markdown("""
            **📖 Financial Performance Analysis:**
            - **MAPE 7.8%**: Excellent price accuracy (industry target: <10%)
            - **R² 0.845**: Explains 84.5% of price variance (strong predictive power)
            - **Direction 75.4%**: Strong trading edge (25.4% above random)
            - **Vol Correlation 0.865**: Excellent volatility tracking for risk management
            - **Sharpe 0.84**: Nearly institutional standards (target: >0.8)
            
            **Economic Interpretation:**
            - **MAE $7,600**: Average prediction error in Singapore dollars
            - **Information Ratio 0.38**: Good alpha generation capability
            - **Max Drawdown 6.9%**: Low risk levels for trading strategy
            """)
