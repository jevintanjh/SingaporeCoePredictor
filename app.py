...
def main():
    ...
    if dataset_info and 'date_range_start' in dataset_info:
        ...
        date_range = f"{data['date'].min().strftime('%d-%m-%Y')} to {data['date'].max().strftime('%d-%m-%YYYY')}"
            ...
            if scheduler_status['next_bidding_dates']:
                ...
                for date_str in scheduler_status['next_bidding_dates'][:3]:
                    try:
                        ...
                        formatted_dates.append(date_obj.strftime("%d-%m-%YYYY"))
                    except:
        ...
        if rankings:
            ...
            for rank_info in rankings:
                ranking_data.append({
                    ...
                    "Last Updated": datetime.fromisoformat(rank_info['last_updated']).strftime("%d-%m-%YYYY %H:%M")
                })
                            ...
                            if 'last_6_scores' in metrics and metrics['last_6_scores']:
                                ...
                                if len(metrics['last_6_scores']) > 0:
                                    ...
                                    for i, (score, date) in enumerate(zip(metrics['last_6_scores'], metrics['last_6_dates'])):
                                        try:
                                            formatted_date = datetime.fromisoformat(date).strftime("%d-%m-%YYYY")
                                        except:
    ...
    model_options = {
        'Fast Directional': 'Fast Directional Forecaster (88.9% accuracy)',
        'Interpretable N-BEATS': 'Interpretable N-BEATS (90.6% accuracy)', 
        'N-BEATSx': 'N-BEATSx (92.7% accuracy)'
    }
